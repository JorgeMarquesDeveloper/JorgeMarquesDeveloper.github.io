...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
   ...
   $upload_dir = "uploads/" . $idCandidato . "/exp_mil_ott/";
   ...
   $new_filename =  $funcao ."." . $file_extension;
   $filename = $upload_dir . $new_filename;
   move_uploaded_file($anexo['tmp_name'], $filename);
   $link_anexo = $upload_dir . $new_filename;

   // Insere os dados na tabela
   $stmt = $conn->prepare("INSERT INTO experiencia_militar_ott (id_cad, local_servico, funcao_desempenhada, data_inicio_servico, data_termino_servico	, anexo) VALUES (?, ?, ?, ?, ?, ?)");
   $stmt->bind_param("isssss", $idCandidato, $local, $funcao, $dataInicio, $dataFim, $link_anexo);

   if ($stmt->execute()) {
       header("Location:system_ott_cad_curso_comp_anxr.php");
       exit();
   } else {
